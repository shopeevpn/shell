...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget --no-check-certificate -N "https://cdn.jsdelivr.net/gh/narakoi/CDN@1.0/brook/brook_linux_amd64" && mv brook_linux_amd64 brook
    else
        wget --no-check-certificate -N "https://cdn.jsdelivr.net/gh/narakoi/CDN@1.0/brook/brook_linux_386"
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/narakoi/CDN@1.0/brook/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/narakoi/CDN@1.0/brook/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
...
if [[ "${action}" == "monitor" ]]; then
    ...
    echo && echo -e "  Brook 端口转发 一键管理脚本修改版_jsdelivr加速版(Blog:kyz.im) ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
